...
Automatic interactive documentation with Swagger UI (from the OpenAPI backend_api_gateway): http://localhost/docs

Alternative automatic documentation with ReDoc (from the OpenAPI backend_api_gateway): http://localhost/redoc
...
```bash
docker-compose logs backend_api_gateway
```
...
From `./backend_api_gateway/app/` you can install all the dependencies with:
...
For example, the directory with the backend_api_gateway code is mounted as a Docker "host volume", mapping the code you change live to the directory inside the container. That allows you to test your changes right away, without having to build the Docker image again. It should only be done during development, for production, you should build the Docker image with a recent version of the backend code. But during development, it allows you to iterate very fast.
...
```console
$ docker-compose exec backend_api_gateway bash
```
